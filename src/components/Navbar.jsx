      ...
      <AnimatePresence>
        {enableNavbar && (
          ...
          >
            ...
            <motion.a className="z-10 custom-cursor" onClick={()=>setEnableNavbar(false)} initial={{x:100,opacity:0}} animate={{x:0,opacity:1,transition:{type:"keyframes",delay:.3}}} exit={{x:100,opacity:0}} href="#About">About</motion.a>
            <motion.a className="z-10 custom-cursor" onClick={()=>setEnableNavbar(false)}  initial={{x:100,opacity:0}} animate={{x:0,opacity:1,transition:{type:"keyframes",delay:.4}}} exit={{x:100,opacity:0}} href="#Skills">Skills</motion.a>
            <motion.a className="z-10 custom-cursor" onClick={()=>setEnableNavbar(false)} initial={{x:100,opacity:0}} animate={{x:0,opacity:1,transition:{type:"keyframes",delay:.5}}} exit={{x:100,opacity:0}} href="#Projects">Projects</motion.a>
            <motion.a className="z-10 custom-cursor"  onClick={()=>setEnableNavbar(false)} initial={{x:100,opacity:0}} animate={{x:0,opacity:1,transition:{type:"keyframes",delay:.6}}} exit={{x:100,opacity:0}} href="#Contact">Contact</motion.a>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
